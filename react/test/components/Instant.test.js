...
describe("Instant", () => {
    const { getByDisplayValue, findByTestId, findByText, getByText, getByTestId } = screen;
    ...
    it("we can change the date from null and when date to date", async () => {
        ...
        const labelText = await findByText("Status Date");
            ...
            expect(
                getByText("2026-01-01T00:00:00.000+00:00")
            ).toBeInTheDocument();
            fireEvent.change(inputField, { target: { value: "01/01/2027" } })
            expect(
                getByText("2027-01-01T00:00:00.000+00:00")
            ).toBeInTheDocument();
    });
    it("Handles change for Hours, minutes, seconds, ms; does not combust.", async () => {
        ...
        fireEvent.change(getByTestId("status_date_time-hour-input"), { target : {value: "1"}})
        fireEvent.change(getByTestId("status_date_time-minute-input"), { target : {value: "2"}})
        fireEvent.change(getByTestId("status_date_time-second-input"), { target : {value: "3"}})
        fireEvent.change(getByTestId("status_date_time-millisecond-input"), { target : {value: "4"}})
        expect(
            getByText("2026-01-01T01:02:03.004+00:00")
        ).toBeInTheDocument();
    });
});
